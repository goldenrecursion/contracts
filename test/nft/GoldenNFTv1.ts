...
const getTokenIdFromEvent = async (receipt: ContractReceipt) => {
  const intrfc = new ethers.utils.Interface(["event Minted(uint256 indexed tokenId, string ceramicId)"]);
...
describe('SharedOwnershipNFT - NFT Component', function () {
  ...
  describe('NFT', function () {
    it('Should test minting/burning', async function () {
      ...
      await expect(GoldenNFTv1.burn(12345)).to.be.revertedWith('burn nonexistent token')
      expect(await GoldenNFTv1._totalSupply()).to.equal('2')
      ...
      await expect(GoldenNFTv1.mint('')).to.be.revertedWith('ceramicId cannot be empty')
      expect(await GoldenNFTv1._totalSupply()).to.equal(mintsNumber - 10);
    });
    it('Should test events', async function () {
      await expect(
        GoldenNFTv1.mint( hash1)
      ).to.emit(GoldenNFTv1, 'Minted')
        .withArgs(0, hash1);
      await expect(
        GoldenNFTv1.mint(hash2)
      ).to.emit(GoldenNFTv1, 'Minted')
        .withArgs(1, hash2);
      await expect(
