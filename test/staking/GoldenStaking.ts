...
describe('GoldenStaking', function () {
  ...
  describe('Access control', function () {
    it('Should test onlyOwner functions', async () => {
      ...
      expect(await GoldenToken.balanceOf(GoldenStaking.address)).to.equal('33333');
      expect(await GoldenToken.balanceOf(owner.address)).to.equal('999999999999999999999966667');
      await GoldenStaking.recoverERC20(GoldenToken.address);
      expect(await GoldenToken.balanceOf(owner.address)).to.equal('1000000000000000000000000000');
    });

    it('Should fail calling onlyOwner functions', async () => {
      await GoldenStaking.transferOwnership(
        address2
      );
      ...
      await expect(GoldenStaking.recoverERC20(GoldenToken.address)).to.be.revertedWith(
        ownableError
      );
    });
    it('Should test deposit/withdraw', async () => {
      ...
      expect(await ethers.provider.getBalance(owner.address)).to.equal(ownerAmount.toString());
      const toSend = 3333
      ...
      const blockStamp = (await ethers.provider.getBlock(receipt.blockNumber)).timestamp
      ...
      expect(await ethers.provider.getBalance(GoldenStaking.address)).to.equal(toSend);
      expect(await GoldenStaking.balances(owner.address)).to.equal(toSend);
      expect(await GoldenStaking.lockedUntilTimes(owner.address)).to.equal(blockStamp + stakingTime);
    ...
    it('Should test events', async function () {
      ...
      const blockStamp = (await ethers.provider.getBlock(ethers.provider.blockNumber)).timestamp
